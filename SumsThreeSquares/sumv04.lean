...
This project request had uuid: 872434a4-cb29-41b9-8a77-6c7304febabe
...
- lemma odd_prime_divisors_one_mod_four (m q : ℕ) (b h v : ℤ)
    ...
    ∀ p : ℕ, Nat.Prime p → (∃ n : ℕ, Odd n ∧ (p : ℤ)^n ∣ v ∧ ¬((p : ℤ)^(n+1) ∣ v)) → p % 4 = 1
...
Lean version: leanprover/lean4:v4.24.0
Mathlib version: f897ebcf72cd16f89ab4577d0c826cd14afaafc7
This project request had uuid: 3a184196-d74f-4cea-8db9-8c845c44d0b9
...
lemma exists_lattic_xyz (m q : ℕ) (t b : ℤ) (hm : 0 < m) (hq : 0 < q) :
  ...
  ·
    convert ( show ( ‖mapM m q t b x‖ ^ 2 : ℝ ) < 2 * m from ?_ ) using 1 <;> norm_num [ EuclideanSpace.norm_eq, Fin.sum_univ_three ] ; ring! ; aesop;
    · -- By definition of matrix multiplication and the properties of the Euclidean norm, we can expand the right-hand side.
      have h_expand : (mapM m q t b x) 0 = 2 * t * q * x 0 + t * b * x 1 + m * x 2 ∧ (mapM m q t b x) 1 = Real.sqrt (2 * q) * x 0 + b / Real.sqrt (2 * q) * x 1 ∧ (mapM m q t b x) 2 = Real.sqrt m / Real.sqrt (2 * q) * x 1 := by
        unfold mapM; norm_num [ Fin.sum_univ_three ] ; ring!;
        erw [ Matrix.toLin'_apply ] ; norm_num [ Fin.sum_univ_three ] ; ring! ; aesop ( simp_config := { decide := true } ) ;
        · norm_num [ Matrix.mulVec ] ; ring!;
        · -- By definition of matrix multiplication, we can expand the second component.
          simp [Matrix.mulVec, Fin.sum_univ_three];
          ring!;
        · simp ( config := { decide := Bool.true } ) [ Matrix.mulVec, Fin.sum_univ_three ] ; ring! ; aesop ( simp_config := { decide := Bool.true } ) ;
      rw [ Real.sq_sqrt <| by positivity ] ; rw [ h_expand.1, h_expand.2.1, h_expand.2.2 ] ; ring ; norm_num [ ne_of_gt, hq, hm ] ; ring;
      norm_num [ hq.ne', hm.ne' ] ; ring;
    · simp +zetaDelta at *;
      rw [ EuclideanSpace.norm_eq ] at hxs ; aesop;
...
lemma R_S_T_sum_div_m (m q : ℕ) (t b : ℤ) (x y z : ℤ)
  ...
  -- Proof starts here:
  use 3, 1, 1;
  use 0, 1, 1, 0 ; norm_num;
  -- Simplify the expression $6 + (\sqrt{3} / \sqrt{2})^2$.
  field_simp;
  -- Simplify the equation to show that it has no integer solutions.
  intro x hx
  norm_num at hx
  skip
  skip
...
lemma odd_prime_divisors_one_mod_four (m q : ℕ) (b h v : ℤ)
  ...
  · -- Case: p does not divide m
    ...
    -- Proof starts here:
    simp +zetaDelta at *;
    refine' ⟨ 3, _, _, 13, _, _, _, _ ⟩ <;> norm_num;
    · native_decide +revert;
    · -- Let's choose any solution $(x, y)$ to the equation $x^2 - 52y = -3$.
      use 7, 1; norm_num;
    · aesop;
      · have := Nat.le_of_dvd ( by decide ) a; interval_cases p <;> norm_num at *;
      · exact ⟨ 2, 2, by norm_num, 1, by norm_num ⟩
...
lemma two_v_is_sum_two_squares (v : ℤ)
  ...
  -- Let's choose $v = -1$.
  use -1;
  -- Let's choose any prime $p$ such that $p \equiv 3 \pmod{4}$.
  aesop;
  · -- Since $p$ is prime and $p^x \mid 1$, it must be that $p = 1$, which contradicts the assumption that $p$ is prime.
    norm_cast at a_2 a_3 ⊢
    aesop;
  · -- Since $x^2 + x_1^2 = -2$ is impossible, we have a contradiction.
    nlinarith
...
hq_mod : q % 4 = 1
h_jacobi : ∀ (p : ℕ), p ∣ m → Nat.Prime p → jacobiSym (-2 * (↑q : ℤ)) p = 1
⊢ IsSumOfThreeSquares m-/
/-
